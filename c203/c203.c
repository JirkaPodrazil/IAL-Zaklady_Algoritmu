...
void Queue_Init( Queue *queue ) {
	//osetreni ze parametr queue se nerovna NULL
	if (queue == NULL){
		Queue_Error(QERR_INIT);
	}

	//inincializase kazdeho prvku pole danne fronty na "*"
	for (int i; i = 0; i < MAX_QUEUE - 1){
		queue->array[i] = '*';
	}

	//nastaveni indexu zacatku fronty a prvniho volneho mista
	queue->firstIndex = 0;
	queue->freeIndex = 0;
}
...
int nextIndex( int index ) {
	//vrati index+1 pokud neni index roven maximalni velikosti pole, pak vrati 1
	return (index % MAX_QUEUE)+1;
}
...
int Queue_IsEmpty( const Queue *queue ) {
	//pokud je prvni index pole roven nule, pole je prazne
	return (0 == queue->firstIndex);
}
...
int Queue_IsFull( const Queue *queue ) {
	//pokud je fronta plna, freeIndex bude roven MAX_QUEUE
	return (nextIndex(queue->freeIndex)-1);
}
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
	//kontrola ze fronta neni prazdna
...
void Queue_Remove( Queue *queue ) {
	...
	//zvetsi index o jedna pokud neni maximalni
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	...
	//dosadi do dataPtr hodnotu prvniho prvku fronty
	Queue_Front(queue, dataPtr);
	//"smaze" prvni prvek fronty pomoci nextIndex
...
void Queue_Enqueue( Queue *queue, char data ) {
	//kontrola ze fronta neni plna
	if (Queue_IsFull(queue)){
		Queue_Error(QERR_ENQUEUE);
		return;
	}
	//priradi data na prvni volny index a nasledne zvetsi jeho hodnotu
