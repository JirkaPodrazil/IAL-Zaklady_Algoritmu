...
void List_Dispose( List *list ) {
	//opakovane maze prvni prvek seznamu dokud neni seznam prazdny
...
void List_InsertFirst( List *list, int data ) {
	//allokuje prostor pro novy element seznamu
	...
	//vlozeni dat do noveho inicializovaneho prvku
...
void List_GetFirst( List *list, int *dataPtr ) {
	//kontola ze list neni prazdny
	if(list->firstElement == NULL){
		List_Error();
		return;
	}
	//vlozeni dat prvniho prvku do pomocne promenne
...
void List_DeleteFirst( List *list ) {
	...
	if (list->firstElement == NULL) {
		...
		//docasne vytvori element, ktery je pouzity pro zmenu ukazatele nextElement
		//docasny i prvni element jsou uvolneny soucasne
...
void List_DeleteAfter( List *list ) {
	//kontrola aktivity seznamu (aktivni prvek nesmi byt posledni)
	if (list->activeElement == NULL || list->activeElement->nextElement == NULL){
		return;
	}
	else{
		//pomocny prvek k predani ukazatele nextElement (nextElement bude smazan a doslo by ke ztrate navaznosti seznamu)
		struct ListElement *helpElem = list -> activeElement->nextElement->nextElement;
		free(list->activeElement->nextElement);
		list->activeElement->nextElement = helpElem;
	}
}
...
void List_InsertAfter( List *list, int data ) {
	...
	//dosazeni dat a ukazatelu dalsiho prku
	newElement->data = data;
...
void List_GetValue( List *list, int *dataPtr ) {
	//kontrola aktivity seznamu
	if (list->activeElement != NULL){
		//vraci ukazatel na aktivni prvek seznamu
